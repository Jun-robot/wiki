...
- バッチサイズ
    ...
    - データセット全ての損失を計算するのは現実的でないことが多い
    - 勾配を計算する一般的な手法
        - 確率的勾配効果法(SGD)
            - イテレーションごとに1つのサンブルのみを使用する
            - 十分な反復回数を取れば機能する？
        - ミニバッチSGD
            - フルバッチとSGDの折衷案
            - 予め決めたバッチサイズNのexampleをランダムに選択して、勾配を平均化してから、パラメーターを更新する
    - 損失曲線?のノイズについて
        - 選択されたexapmleが外れ値だと大きなノイズがのる
        - ある程度のノイズは、モデルの汎化を改善し、NNで最適なパラメーターを見つけるのに役立つ
- エポック
    - 1エポック = モデルがトレーニングデータセットのすべての例を一回処理すること
        - 1000個のサンプルを含むトレーニングセットで、100このミニバッチSGDの場合、10回のイテレーションが必要
    - エポック数が多いほどモデルの精度は向上するが、トレーニングにかかる時間も長くなる

プログラムミング演習
- Correlation Matrix　相関行列
    - Correlation Value 相関係数
        - 1: 片方が上がれば、もう片方も上がる
        - -1: 片方が上がれば、もう片方は下がる
    - 一般的に相関係数が大きい方が、predictive powerは大きくなる
        - 学習につかうFeatureを決めるのに役立つ
    - それぞれの特徴量ごとにプロットするのもわかりやすくて良い。
- 学習させる
    - hyper parameterを指定する
        - 1epochでは訓練データをすべて学習する。
        - batch sizeはいくつのデータをまとめて損失計算するか
        - 訓練データ1000件、batch sizeが50なら、20batch行われる
        - 各batchでは
            - 予想する
            - 損失計算
            - パラメーターを動かす方向を決める
            - パラメーターを更新する
    - 2つ以上のfeatureからトレーニングする
        - 学習に使用するfeatureのスケールはできるだけ揃えた方が良い
